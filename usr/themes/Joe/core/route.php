...
function _getPost($self)
{
    ...
    if (!preg_match('/^\d+$/', $page)) {
        return Typecho_Response::getInstance()->throwJson(array("data" => "Yêu cầu bất hợp pháp! Bị chặn!"));
    }
...
function _handleViews($self)
{
    ...
    /* sql注入校验 */
    if (!preg_match('/^\d+$/',  $cid)) {
        return Typecho_Response::getInstance()->throwJson(array("code" => 0, "data" => "Yêu cầu bất hợp pháp! Bị chặn!"));
    }
    $db = Typecho_Db::get();
...
function _getRecord($self)
{
    ...
    curl_close($ch);
    $res = str_replace([' ', "\n", "\r"], '', $output);
    if (strpos($res, "Xin lỗi, không tìm thấy") || strpos($res, "Tìm thấy khoảng 0 kết quả liên quan") || strpos($res, "URL không được tìm thấy") || strpos($res, "Xin lỗi không tìm thấy")) {
        Typecho_Response::getInstance()->throwJson(array("data" => "Không bao gồm"));
    } else {
        Typecho_Response::getInstance()->throwJson(array("data" => "Đã bao gồm"));
    }
}
...
function _getMaccmsList($self)
{
    ...
    if ($cms_api) {
        ...
        if ($res['code'] === 1) {
            ...
            Typecho_Response::getInstance()->throwJson([
                "code" => 0,
                "data" => "Thu thập thông tin không thành công! Vui lòng liên hệ với tác giả!"
            ]);
        }
    } else {
        Typecho_Response::getInstance()->throwJson([
            "code" => 0,
            "data" => "API CMS của Apple nền chưa được điền vào!"
        ]);
    }
}
...
function _getServerStatus($self)
{
    ...
    if (!$api_panel) return Typecho_Response::getInstance()->throwJson([
        "code" => 0,
        "data" => "Địa chỉ bảng chùa không được điền!"
    ]);
    if (!$api_sk) return Typecho_Response::getInstance()->throwJson([
        "code" => 0,
        "data" => "Phím giao diện chùa không được điền!"
    ]);
    $request_time = time();
...
function _getArticleFiling($self)
{
    ...
    $page = Typecho_Request::getInstance()->page;
    $pageSize = 8;
    if (!preg_match('/^\d+$/', $page)) return Typecho_Response::getInstance()->throwJson(array("data" => "Yêu cầu bất hợp pháp! Bị chặn!"));
    if ($page == 0) $page = 1;
    ...
    $prefix = $db->getPrefix();
    $result = [];
    $sql = "SELECT FROM_UNIXTIME(created, 'tháng %m năm %Y') as date FROM `{$prefix}contents` WHERE created < {$time} AND (password is NULL or password = '') AND status = 'publish' AND type = 'post' GROUP BY FROM_UNIXTIME(created, 'tháng %m năm %Y') DESC LIMIT {$pageSize} OFFSET {$offset}";
