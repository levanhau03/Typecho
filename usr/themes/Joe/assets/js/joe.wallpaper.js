document.addEventListener('DOMContentLoaded', () => {
    ...
    $.ajax({
        ...
        success(res) {
            if (res.code !== 1) return $('.joe_wallpaper__type-list').html('<li class="error">Thu thập thông tin hình nền không thành công! Vui lòng liên hệ với tác giả!</li>');
            if (!res.data.length) return $('.joe_wallpaper__type-list').html(`<li class="error">Không có dữ liệu!</li>`);
            let htmlStr = '';
                ...
                res.data.forEach(_ => {
                    htmlStr += `
                        <a class="item animated bounceIn" data-fancybox="gallery" href="${_.url}">
                            <img width="100%" height="100%" class="lazyload" src="${Joe.LAZY_LOAD}" data-src="${_.img_1024_768 || _.url}" alt="Hình nền">
                        </a>`;
                });
    ...
    function initPagination() {
        ...
        if (queryData.start / queryData.count !== 0) {
            htmlStr += `
                <li class="joe_wallpaper__pagination-item" data-start="0">Trang chủ</li>
