    ...
    function initVideoList() {
        ...
        $.ajax({
            ...
            success(res) {
                ...
                if (!res.data.class.length) return $('.joe_video__type-list').html(`<li class="error">Không có dữ liệu!</li>`);
                let htmlStr = '<li class="item" data-t="">Tất cả</li>';
        ...
        function renderDom() {
            ...
            $.ajax({
                ...
                success(res) {
                    if (res.code !== 1) return $('.joe_video__list-item').css('display', 'block').html('<p class="error">Tải dữ liệu không thành công! Hãy kiểm tra!</p>');
                    if (!res.data.list.length) {
                        $('.joe_video__list-item').css('display', 'block').html('<p class="error">Không có dữ liệu!</p>');
                    } else {
                        let htmlStr = '';
        ...
        function initPagination() {
            ...
            if (queryData.pg != 1) {
                htmlStr += `
            		<li class="joe_video__pagination-item" data-pg="1">Trang chủ</li>
            ...
            if (queryData.pg < pagecount) htmlStr += `<li class="joe_video__pagination-item" data-pg="${pagecount}">Trang cuối</li>`;
    ...
    function initVideoDetail() {
        ...
        $.ajax({
            ...
            success(res) {
                ...
                if (!res.data.list.length) return $('.joe_video__detail-info').html(`<p class="error">Bắt dữ liệu bất thường! Hãy kiểm tra!</p>`);
                ...
                $('.joe_video__detail-info').html(`
					...
					<dl class="description">
						...
						<dd><span class="muted">Kiểu:</span><span class="text">${item.vod_class || 'không xác định'}</span></dd>
						<dd><span class="muted">Diễn viên:</span><span class="text">${item.vod_actor || 'không xác định'}</span></dd>
						<dd><span class="muted">Giám đốc:</span><span class="text">${item.vod_director || 'không xác định'}</span></dd>
						<dd><span class="muted">Giới thiệu:</span><span class="text">${getContent(item)}</span></dd>
					</dl>
				`);
                /* 设置视频播放标题 */
                $('.joe_video__player .joe_video__contain-title').html('Đang phát:' + item.vod_name);
                /* 设置播放链接 */
                let parseList = str => {
                    let htmlStr = '';
                    let arr = str.split('#');
                ...
                for (let [key, value] of maps) {
                    ...
                    htmlStr += `
						<div class="joe_video__source joe_video__contain">
							<div class="joe_video__contain-title">Đường chơi ${index}</div>
