document.addEventListener('DOMContentLoaded', () => {
	...
	/* 获取本篇文章百度收录情况 */
	{
		$.ajax({
			...
			success(res) {
				if (res.data && res.data === 'Đã bao gồm') {
					$('#Joe_Baidu_Record').css('color', '#67C23A');
					$('#Joe_Baidu_Record').html('Đã bao gồm');
				} else {
					/* 如果填写了Token，则自动推送给百度 */
					if (Joe.BAIDU_PUSH) {
						$('#Joe_Baidu_Record').html('<span style="color: #E6A23C">Không bao gồm, đẩy...</span>');
						const _timer = setTimeout(function () {
							$.ajax({
								...
								success(res) {
									if (res.data.error) {
										$('#Joe_Baidu_Record').html('<span style="color: #F56C6C">Đẩy không thành công, vui lòng kiểm tra!</span>');
									} else {
										$('#Joe_Baidu_Record').html('<span style="color: #67C23A">Đẩy thành công!</span>');
									}
								}
							});
							clearTimeout(_timer);
		...
		if (!flag) {
			$.ajax({
				...
				success(res) {
					...
					$('#Joe_Article_Views').html(`${res.data.views} xem`);
					viewsArr.push(cid);
