...
<div class="joe_comment">
    <h3 class="joe_comment__title">Bình luận <?php if ($this->allow('comment') && $this->options->JCommentStatus !== "off") : ?>(<?php $this->commentsNum(); ?>)<?php endif; ?></h3>
    ...
    <?php if ($this->hidden) : ?>
        <div class="joe_comment__close">
            ...
            <span>Bài viết hiện tại được bảo vệ bằng mật khẩu và không thể bình luận</span>
        </div>
    <?php else : ?>
        <?php if ($this->allow('comment') && $this->options->JCommentStatus !== "off") : ?>
            <div id="<?php $this->respondId(); ?>" class="joe_comment__respond">
                <div class="joe_comment__respond-type">
                    <button class="item" data-type="draw">Chế độ vẽ</button>
                    <button class="item active" data-type="text">Chế độ văn bản</button>
                </div>
                <form method="post" class="joe_comment__respond-form" action="<?php $this->commentUrl() ?>" data-type="text">
                    <div class="head">
                        <div class="list">
                            <input type="text" value="<?php $this->user->hasLogin() ? $this->user->screenName() : $this->remember('author') ?>" autocomplete="off" name="author" maxlength="16" placeholder="Hãy nhập một nickname..." />
                        </div>
                        <div class="list">
                            <input type="text" value="<?php $this->user->hasLogin() ? $this->user->mail() : $this->remember('mail') ?>" autocomplete="off" name="mail" placeholder="Nhập email của bạn..." />
                        </div>
                        <div class="list">
                            <input type="text" autocomplete="off" name="url" placeholder="Nhập URL (không bắt buộc)..." />
                        </div>
                    </div>
                    <div class="body">
                        <textarea class="text joe_owo__target" name="text" value="" autocomplete="new-password" placeholder="Nói gì đó, nhấp vào trên cùng bên phải để chuyển sang vẽ thử?"></textarea>
                        <div class="draw" style="display: none;">
                            <ul class="line">
                                <li data-line="3">nhỏ</li>
                                <li data-line="5" class="active">vừa</li>
                                <li data-line="8">lớn</li>
                            </ul>
                            <ul class="color">
                    ...
                    <div class="foot">
                        ...
                        <div class="submit">
                            <span class="cancle joe_comment__cancle">Hủy bỏ</span>
        ...
        <?php else : ?>
            <div class="joe_comment__close">
                ...
                <?php if ($this->options->JCommentStatus === "off") : ?>
                    <span>Blogger đã tắt nhận xét trên tất cả các trang</span>
                <?php else : ?>
                    <span>Blogger đã tắt bình luận trên trang hiện tại</span>
                <?php endif; ?>
            </div>
        <?php endif; ?>
    <?php endif; ?>
...
{ ?>
    <li class="comment-list__item">
        <div class="comment-list__item-contain" id="<?php $comments->theId(); ?>">
            <div class="term">
                <img width="48" height="48" class="avatar lazyload" src="<?php _getAvatarLazyload() ?>" data-src="<?php _getAvatarByMail($comments->mail); ?>" alt="Hình đại diện" />
                <div class="content">
                    <div class="user">
                        <span class="author"><?php $comments->author(); ?></span>
                        <?php if ($comments->authorId === $comments->ownerId) : ?>
                            <i class="owner">Tác giả</i>
                        <?php endif; ?>
                        <?php if ($comments->status === "waiting") : ?>
                            <em class="waiting">(Bình luận đang xem xét ...)</em>
                        <?php endif; ?>
                    ...
                    <div class="handle">
                        ...
                        <span class="reply joe_comment__reply" data-id="<?php $comments->theId(); ?>" data-coid="<?php $comments->coid(); ?>">
                            <i class="icon fa fa-pencil" aria-hidden="true"></i>Trả lời
                        </span>
                    </div>
                </div>
            </div>
